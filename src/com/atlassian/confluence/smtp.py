...
def ntlm_authenticate(smtp, username, password):
    ...
    """
    code, response = smtp.docmd("AUTH", "NTLM " + ntlm.create_NTLM_NEGOTIATE_MESSAGE(username))
    if code != 334:
    ...
    code, response = smtp.docmd("", ntlm.create_NTLM_AUTHENTICATE_MESSAGE(challenge, user_parts[1], user_parts[0], password, flags))
    if code != 235:
