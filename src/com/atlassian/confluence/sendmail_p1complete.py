...
R_TEAM_MailAddr = 'neal.tao@netis.com'
...
def getContentTables(content):
    ...
    'r_team' : makeTableContentList(tables[11])
    }
    return team_infos
...
def getScrumSP(team, sprint):
    ...
    if len(sprint_info)< 17:
        sprint_P1 = 'completed'
    else:
        sprint_P1 = sprint_info[13]
...
def get_team_sprint_info(day):
    num = int(parse_sprint_date(day)) - 1
    ...
    if blank_teams:
        for team in blank_teams:
            if team in ['a_team','b_team','c_team']:
                ...
                send_mail_addr.append(R_TEAM_MailAddr)
            else:
                print 'Error team'
...
def send_mail(mail_to, mail_from,server="localhost"):
    ...
    msg['Subject'] = '请更新Sprint P1 Complete'

    text = """
        <html>
            <body>
                <p>请各位更新上个Sprint的P1 Complete，谢谢！
...
if __name__ == "__main__":
    ...
    if parse_sprint_date(today):
        ...
        pass
    # send_mail(['sam.zhu@netis.com','wills.fei@netis.com'],['sherry.zheng@netis.com'])
    # mailto = get_mail_addr(today)
    # mailto.append('sherry.zheng@netis.com')
